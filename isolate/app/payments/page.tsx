    ...
    <CustomerLayout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white">Bill Pay & Scheduled Payments</h1>
            ...
            >
              <div>
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs font-semibold text-[#38bdf8] uppercase tracking-wider bg-[#38bdf8]/10 px-2 py-0.5 rounded">
                    {payee.category}
                  </span>
                  <span className="text-[10px] text-[#64748b] font-mono">Acc: {payee.accountNumber}</span>
                </div>

                <h3 className="text-lg font-bold text-white mb-1">{payee.name}</h3>
              ...
              <div className="pt-4 border-t border-[#1e293b] flex items-center justify-between">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-[#38bdf8] hover:bg-[#0ea5e9] text-[#0b0f19] text-xs font-semibold rounded-xl transition-all"
                >
                  Pay Now
                </button>
              </div>
        ...
        {selectedPayeeId && (
          <div className="fixed inset-0 z-50 bg-[#0b0f19]/80 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="w-full max-w-md bg-[#161e2e] border border-[#1e293b] rounded-2xl p-6 sm:p-8 shadow-2xl">
              {!success ? (
                <>
                  <h2 className="text-lg font-bold text-white mb-1">Pay Bill: {selectedPayee?.name}</h2>
                  <p className="text-xs text-[#94a3b8] mb-4">Electronic remittance to account {selectedPayee?.accountNumber}</p>

                  <form onSubmit={handlePay} className="space-y-4">
                    <div>
                      <label className="block text-xs font-medium text-[#94a3b8] uppercase mb-2">Payment Amount ($ USD)</label>
                      <input
                        type="number"
                        step="0.01"
                        required
                    ...
                    <div>
                      <label className="block text-xs font-medium text-[#94a3b8] uppercase mb-2">Send Date</label>
                      <input
                        type="date"
                        required
                        value={payDate}
                    ...
                    <div className="flex items-center justify-end gap-3 pt-4 border-t border-[#1e293b]">
                      ...
                      <button
                        type="submit"
                        disabled={loading}
                        className="px-5 py-2.5 bg-[#38bdf8] text-[#0b0f19] font-semibold text-xs rounded-xl hover:bg-[#0ea5e9]"
                      >
                ...
                <div className="text-center py-4 space-y-3">
                  ...
                  <h3 className="text-lg font-bold text-white">Payment Dispatched</h3>
                  <p className="text-xs text-[#94a3b8]">
                    ${parseFloat(amount).toFixed(2)} sent to {selectedPayee?.name}. Confirmation: <span className="font-mono text-white">BP-{Date.now().toString().slice(-6)}</span>
                  </p>
                  <button
                    onClick={() => setSelectedPayeeId(null)}
