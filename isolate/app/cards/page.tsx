      ...
      <section className="pt-32 pb-20 px-4">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <h1 className="text-4xl sm:text-5xl font-bold mb-6">Card <span className="crestline-text-gradient">Products</span></h1>
            <p className="text-lg text-[#94a3b8] max-w-2xl mx-auto">Choose the card that fits your lifestyle.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { name: "Crestline Platinum Card", type: "Credit Card", limit: "$25,000 limit", apr: "19.99% APR", rewards: "3x points on travel & dining", features: ["No annual fee", "0% intro APR 15 months", "Cell phone protection", "Travel insurance", "Contactless payments"], gradient: "from-[#38bdf8] to-[#0ea5e9]" },
              ...
              <div key={i} className="rounded-2xl overflow-hidden border border-[#1e293b] bg-[#161e2e]/60">
                <div className={`h-32 bg-gradient-to-br ${card.gradient} p-6 flex flex-col justify-between`}>
                  ...
                  <div>
                    <p className="text-sm font-medium text-white/90">**** **** **** {(4501 + i).toString().padStart(4, '0')}</p>
                    <p className="text-xs text-white/60 mt-1">Alex Morgan</p>
                  </div>
                </div>
                <div className="p-6">
                  <h3 className="text-lg font-bold text-[#f8fafc] mb-1">{card.name}</h3>
                  <div className="flex gap-3 mb-4">
                    <span className="text-xs text-[#38bdf8]">{card.limit}</span>
                    <span className="text-xs text-[#94a3b8]">•</span>
                    <span className="text-xs text-[#94a3b8]">{card.apr}</span>
                  </div>
                  ...
                  <button className="mt-6 w-full py-2.5 rounded-xl bg-[#38bdf8]/10 text-[#38bdf8] text-sm font-semibold hover:bg-[#38bdf8]/20 transition-colors">
                    Apply Now
                  </button>
                </div>
              </div>
