  ...
  return (
    <CustomerLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-white">Investment Treasury & Wealth</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-[#161e2e] border border-[#1e293b] rounded-2xl p-6 shadow-xl">
            <span className="text-xs font-medium text-[#94a3b8] uppercase">Portfolio Value</span>
          ...
          <div className="bg-[#161e2e] border border-[#1e293b] rounded-2xl p-6 shadow-xl">
            <span className="text-xs font-medium text-[#94a3b8] uppercase">Unrealized P&L</span>
            <div className="text-3xl font-bold font-mono text-emerald-400 mt-1">
              +$29,840.00
            </div>
            <span className="text-xs text-[#94a3b8] mt-1 block">Dividend reinvestment active</span>
          </div>

          <div className="bg-[#161e2e] border border-[#1e293b] rounded-2xl p-6 shadow-xl">
            <span className="text-xs font-medium text-[#94a3b8] uppercase">Risk Profile</span>
            <div className="text-2xl font-bold text-[#38bdf8] mt-1">
              Moderate Growth
            </div>
            <span className="text-xs text-[#94a3b8] mt-1 block">Quarterly automated rebalancing</span>
          </div>
        </div>

        {/* Holdings Table */}
        <div className="bg-[#161e2e] border border-[#1e293b] rounded-2xl shadow-xl overflow-hidden p-6">
          <h2 className="text-base font-bold text-white mb-4">Core Portfolio Holdings</h2>
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-[#1e293b]/50">
                {portfolioHoldings.map((h) => (
                  <tr key={h.symbol} className="hover:bg-[#0b0f19]/30 transition-colors">
                    <td className="py-3.5 font-medium text-white">
                      <div className="flex items-center gap-2">
                        <span className="font-mono font-bold text-[#38bdf8] bg-[#38bdf8]/10 px-2 py-0.5 rounded">
                          {h.symbol}
                        </span>
                        <span className="text-[#94a3b8] truncate max-w-xs">{h.name}</span>
                      </div>
                    </td>
                    <td className="py-3.5 text-white font-mono">{h.allocation}</td>
                    <td className="py-3.5 text-[#94a3b8] font-mono">{h.shares}</td>
                    <td className="py-3.5 font-mono font-bold text-white">{formatCurrency(h.value)}</td>
                    <td className="py-3.5 text-right font-mono font-bold text-emerald-400">{h.returnPct}</td>
                  </tr>
                ))}
              </tbody>
            </table>
