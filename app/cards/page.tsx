      ...
      <section className="pt-32 pb-20 px-4">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <h1 className="text-4xl sm:text-5xl font-bold mb-6">Card <span className="chase-text-gradient">Products</span></h1>
            <p className="text-lg text-gray-500 max-w-2xl mx-auto">Choose the card that fits your lifestyle.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { name: "Chase Platinum Card", type: "Credit Card", limit: "$25,000 limit", apr: "19.99% APR", rewards: "3x points on travel & dining", features: ["No annual fee", "0% intro APR 15 months", "Cell phone protection", "Travel insurance", "Contactless payments"], gradient: "from-[#38bdf8] to-[#0ea5e9]" },
              { name: "Chase Rewards Card", type: "Debit Card", limit: "Your balance", apr: "No interest", rewards: "1% cash back on all purchases", features: ["No fees", "Free ATM worldwide", "Real-time alerts", "Freeze/unfreeze", "Virtual card available"], gradient: "from-[#10b981] to-[#059669]" },
              { name: "Chase Virtual Card", type: "Virtual Card", limit: "Customizable", apr: "N/A", rewards: "Perfect for online shopping", features: ["Instant creation", "Single-use option", "Spending limits", "Merchant locks", "Full control"], gradient: "from-[#8b5cf6] to-[#6d28d9]" },
            ].map((card, i) => (
              <div key={i} className="rounded-2xl overflow-hidden border border-gray-200 bg-gray-100/60">
                ...
                <div className="p-6">
                  ...
                  <div className="flex gap-3 mb-4">
                    <span className="text-xs text-[#0a4fa6]">{card.limit}</span>
                  ...
                  <ul className="space-y-2">
                    {card.features.map((f, j) => (
                      <li key={j} className="flex items-center gap-2 text-sm text-gray-500">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#38bdf8" strokeWidth="2"><path d="M20 6L9 17l-5-5" /></svg>
                        {f}
                      </li>
                    ))}
                  </ul>
                  <button className="mt-6 w-full py-2.5 rounded-xl bg-[#0a4fa6]/10 text-[#0a4fa6] text-sm font-semibold hover:bg-[#0a4fa6]/20 transition-colors">
                    Apply Now
                  </button>
                </div>
              </div>
